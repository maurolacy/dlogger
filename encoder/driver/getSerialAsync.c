...
void signal_handler_IO(int signal,
    siginfo_t *si,
    void *ctx); /* definition of signal handler */
...
int main(int argc,
    char **argv)
{
    int opt, x = 0, p = 0, v = 0, nsecs = 60, nmins = 0, res, restot = 0, mintot = 0, sec3 = 0, mins = 0;
    time_t oldsecs, secs;
    struct tm t;
    ...
    {
        //usleep(100000);
        usleep(400); // 60 rpm max
        gettimeofday(&tv, NULL);

        if (wait_flag == FALSE)
        {
            // check DCD (B) is asserted
            /* get the serial port's status */
            int status;
            int dir=1;
            ioctl(fd, TIOCMGET, &status);
            if (status&TIOCM_CAR) // CCW?
                dir = -1;
            res = read(fd, buf, 255);
            buf[res] = '\0';
            //for (i=0; i<res; i++)
